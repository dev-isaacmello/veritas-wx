...
# or piece by piece:
uv run python scripts/ingest_obs.py                  # INMET bulk -> OBS_V1 parquet
uv run python scripts/run_qc.py                      # QC + station curation cut
uv run python scripts/scale_ingest.py --model gfs --all-months   # forecasts (resumable)
...
See [CONTRIBUTING.md](CONTRIBUTING.md). The short version: quality over quantity, no estimate
without a CI, and nothing enters the confirmatory registry without a versioned amendment.
